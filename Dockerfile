...
ENV PIPFLAGS="--ignore-installed --no-cache-dir -U --find-links https://wheel-index.linuxserver.io/alpine-3.15/"
...
RUN \
  ...
  pip install ${PIPFLAGS} -r /app/htpcmanager/requirements.txt && \
  echo "**** cleanup ****" && \
  apk del --purge \
    build-dependencies && \
  rm -rf \
